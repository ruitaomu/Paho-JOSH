...
public class MqttTopic {
	...
	public static void validate(String topicString, boolean wildcardAllowed) {
		...
		if (topicLen < MIN_TOPIC_LEN || topicLen > MAX_TOPIC_LEN) {
			throw new IllegalArgumentException("Invalid topic length, should be in range[" +
					new Integer(MIN_TOPIC_LEN) +
					", " +
					new Integer(MAX_TOPIC_LEN) +
					"]!"
					);
		}
		...
		if (wildcardAllowed) {
			...
			if (Strings.countMatches(topicString, MULTI_LEVEL_WILDCARD) > 1
					|| (Strings.containsAny(topicString, MULTI_LEVEL_WILDCARD) && !topicString
